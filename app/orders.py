...
* ``OptionLegRequest`` carries ``side`` and ``position_intent``; for a new short vertical
  the short leg is ``sell_to_open`` and the long leg ``buy_to_open``.
* For an mleg limit order a **positive** ``limit_price`` is a net debit, a **negative**
  one a net credit — a credit spread is submitted with a negative limit.
* The submitted legs are the candidate's own chain symbols (``short_symbol`` /
...
one. Everything fails closed: a candidate that cannot be sized, priced or submitted
produces a typed "do not send" outcome or a raised error the caller records — never a
best-effort order.
"""
